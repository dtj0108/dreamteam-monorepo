"use client"

import * as React from "react"
import { Button } from "@/components/ui/button"
import { Calendar } from "@/components/ui/calendar"
import { Popover, PopoverContent, PopoverTrigger } from "@/components/ui/popover"
import {
  Select,
  SelectContent,
  SelectItem,
  SelectTrigger,
  SelectValue,
} from "@/components/ui/select"
import { Switch } from "@/components/ui/switch"
import { Label } from "@/components/ui/label"
import { Checkbox } from "@/components/ui/checkbox"
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar"
import { Badge } from "@/components/ui/badge"
import {
  CalendarIcon,
  Users,
  ChevronDown,
  Check,
  Tag,
  Building2,
  MapPin,
  Sparkles,
  X,
  Filter,
} from "lucide-react"
import { cn } from "@/lib/utils"
import { format, subDays, startOfMonth, endOfMonth, startOfQuarter, endOfQuarter, startOfYear } from "date-fns"
import type { DateRange } from "react-day-picker"

export interface LeadTag {
  id: string
  name: string
  color: string
}

export interface LeadPipeline {
  id: string
  name: string
  is_default: boolean
  stages?: LeadPipelineStage[]
}

export interface LeadPipelineStage {
  id: string
  name: string
  color: string | null
  position: number
}

export interface WorkspaceMember {
  id: string
  profileId: string
  name: string
  displayName: string | null
  avatarUrl: string | null
  email: string
}

export interface LeadsFilters {
  dateRange: DateRange | undefined
  pipelineId: string | null
  stageId: string | null
  statusIds: string[]
  industries: string[]
  states: string[]
  countries: string[]
  sources: string[]
  assignedTo: string[]
  tagIds: string[]
  hasOpportunities: boolean | null
  hasOpenTasks: boolean | null
  hasContacts: boolean | null
}

interface LeadsFilterBarProps {
  filters: LeadsFilters
  onFiltersChange: (filters: LeadsFilters) => void
  pipelines: LeadPipeline[]
  members: WorkspaceMember[]
  tags: LeadTag[]
  availableIndustries: string[]
  availableStates: string[]
  availableCountries: string[]
  availableSources: string[]
}

const datePresets = [
  { label: "All Time", value: "all", getRange: () => undefined },
  { label: "Today", value: "today", getRange: () => ({ from: new Date(), to: new Date() }) },
  { label: "Last 7 days", value: "7d", getRange: () => ({ from: subDays(new Date(), 7), to: new Date() }) },
  { label: "Last 30 days", value: "30d", getRange: () => ({ from: subDays(new Date(), 30), to: new Date() }) },
  { label: "This Month", value: "month", getRange: () => ({ from: startOfMonth(new Date()), to: endOfMonth(new Date()) }) },
  { label: "This Quarter", value: "quarter", getRange: () => ({ from: startOfQuarter(new Date()), to: endOfQuarter(new Date()) }) },
  { label: "This Year", value: "year", getRange: () => ({ from: startOfYear(new Date()), to: new Date() }) },
]

const leadSources = [
  "Website",
  "Referral",
  "Cold Outreach",
  "LinkedIn",
  "Trade Show",
  "Advertisement",
  "Partner",
  "Other",
]

export function LeadsFilterBar({
  filters,
  onFiltersChange,
  pipelines,
  members,
  tags,
  availableIndustries,
  availableStates,
  availableCountries,
  availableSources,
}: LeadsFilterBarProps) {
  const [datePreset, setDatePreset] = React.useState("all")

  // Get the selected pipeline's stages
  const selectedPipeline = React.useMemo(() => {
    if (!filters.pipelineId) return null
    return pipelines.find(p => p.id === filters.pipelineId) || null
  }, [pipelines, filters.pipelineId])

  const availableStages = React.useMemo(() => {
    if (!selectedPipeline?.stages) return []
    return [...selectedPipeline.stages].sort((a, b) => a.position - b.position)
  }, [selectedPipeline])

  // Count active filters
  const activeFilterCount = React.useMemo(() => {
    let count = 0
    if (filters.dateRange?.from) count++
    if (filters.stageId) count++
    if (filters.industries.length > 0) count++
    if (filters.states.length > 0) count++
    if (filters.countries.length > 0) count++
    if (filters.sources.length > 0) count++
    if (filters.assignedTo.length > 0) count++
    if (filters.tagIds.length > 0) count++
    if (filters.hasOpportunities !== null) count++
    if (filters.hasOpenTasks !== null) count++
    if (filters.hasContacts !== null) count++
    return count
  }, [filters])

  const clearAllFilters = () => {
    onFiltersChange({
      dateRange: undefined,
      pipelineId: filters.pipelineId, // Keep pipeline selection
      stageId: null,
      statusIds: [],
      industries: [],
      states: [],
      countries: [],
      sources: [],
      assignedTo: [],
      tagIds: [],
      hasOpportunities: null,
      hasOpenTasks: null,
      hasContacts: null,
    })
    setDatePreset("all")
  }

  const toggleArrayFilter = (
    key: keyof Pick<LeadsFilters, "industries" | "states" | "countries" | "sources" | "assignedTo" | "tagIds">,
    value: string
  ) => {
    const current = filters[key] || []
    const newValues = current.includes(value)
      ? current.filter((v) => v !== value)
      : [...current, value]
    onFiltersChange({ ...filters, [key]: newValues })
  }

  const handleDatePresetChange = (presetValue: string) => {
    setDatePreset(presetValue)
    const preset = datePresets.find(p => p.value === presetValue)
    if (preset) {
      onFiltersChange({
        ...filters,
        dateRange: preset.getRange(),
      })
    }
  }

  const handleDateRangeChange = (range: DateRange | undefined) => {
    setDatePreset("custom")
    onFiltersChange({
      ...filters,
      dateRange: range,
    })
  }

  const formatDateRange = () => {
    if (!filters.dateRange?.from) {
      return "All Time"
    }
    if (!filters.dateRange.to) {
      return format(filters.dateRange.from, "MMM d, yyyy")
    }
    return `${format(filters.dateRange.from, "MMM d")} - ${format(filters.dateRange.to, "MMM d, yyyy")}`
  }

  const getMultiSelectLabel = (
    items: string[],
    allLabel: string,
    singularLabel: string,
    pluralLabel: string
  ) => {
    if (items.length === 0) return allLabel
    if (items.length === 1) return items[0]
    return `${items.length} ${pluralLabel}`
  }

  // Combine available sources with preset sources
  const allSources = React.useMemo(() => {
    const combined = new Set([...leadSources, ...availableSources])
    return Array.from(combined).sort()
  }, [availableSources])

  return (
    <div className="flex flex-wrap items-center gap-3 px-6 py-3 border-b bg-white/50 dark:bg-slate-900/50">
      {/* Created Date Range */}
      <Popover>
        <PopoverTrigger asChild>
          <Button variant="outline" size="sm" className="h-9 gap-2">
            <CalendarIcon className="h-4 w-4" />
            <span>Created:</span>
            <span className="font-medium">{formatDateRange()}</span>
            <ChevronDown className="h-3 w-3 opacity-50" />
          </Button>
        </PopoverTrigger>
        <PopoverContent className="w-auto p-0" align="start">
          <div className="flex">
            {/* Presets sidebar */}
            <div className="border-r p-2 space-y-1">
              {datePresets.map((preset) => (
                <Button
                  key={preset.value}
                  variant={datePreset === preset.value ? "secondary" : "ghost"}
                  size="sm"
                  className="w-full justify-start text-sm"
                  onClick={() => handleDatePresetChange(preset.value)}
                >
                  {preset.label}
                </Button>
              ))}
            </div>
            {/* Calendar */}
            <div className="p-2">
              <Calendar
                mode="range"
                selected={filters.dateRange}
                onSelect={handleDateRangeChange}
                numberOfMonths={2}
              />
            </div>
          </div>
        </PopoverContent>
      </Popover>

      {/* Pipeline Selector */}
      <Select
        value={filters.pipelineId || "all"}
        onValueChange={(value) => {
          onFiltersChange({
            ...filters,
            pipelineId: value === "all" ? null : value,
            stageId: null, // Reset stage when pipeline changes
          })
        }}
      >
        <SelectTrigger className="h-9 w-[180px]">
          <SelectValue placeholder="All Pipelines" />
        </SelectTrigger>
        <SelectContent>
          <SelectItem value="all">All Pipelines</SelectItem>
          {pipelines.map((pipeline) => (
            <SelectItem key={pipeline.id} value={pipeline.id}>
              {pipeline.name}
              {pipeline.is_default && " (Default)"}
            </SelectItem>
          ))}
        </SelectContent>
      </Select>

      {/* Stage Selector (only when pipeline is selected) */}
      {filters.pipelineId && availableStages.length > 0 && (
        <Select
          value={filters.stageId || "all"}
          onValueChange={(value) =>
            onFiltersChange({ ...filters, stageId: value === "all" ? null : value })
          }
        >
          <SelectTrigger className="h-9 w-[160px]">
            <SelectValue placeholder="All Stages" />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="all">All Stages</SelectItem>
            {availableStages.map((stage) => (
              <SelectItem key={stage.id} value={stage.id}>
                <div className="flex items-center gap-2">
                  <div
                    className="w-2 h-2 rounded-full"
                    style={{ backgroundColor: stage.color || "#6b7280" }}
                  />
                  {stage.name}
                </div>
              </SelectItem>
            ))}
          </SelectContent>
        </Select>
      )}

      {/* Industry Filter */}
      {availableIndustries.length > 0 && (
        <Popover>
          <PopoverTrigger asChild>
            <Button variant="outline" size="sm" className="h-9 gap-2">
              <Building2 className="h-4 w-4" />
              {getMultiSelectLabel(filters.industries, "All Industries", "Industry", "Industries")}
              <ChevronDown className="h-3 w-3 opacity-50" />
            </Button>
          </PopoverTrigger>
          <PopoverContent className="w-64 p-2" align="start">
            <div className="space-y-1 max-h-64 overflow-y-auto">
              {availableIndustries.map((industry) => {
                const isSelected = filters.industries.includes(industry)
                return (
                  <div
                    key={industry}
                    className="flex items-center gap-2 p-2 rounded-md hover:bg-muted cursor-pointer"
                    onClick={() => toggleArrayFilter("industries", industry)}
                  >
                    <Checkbox checked={isSelected} />
                    <span className="text-sm truncate flex-1">{industry}</span>
                    {isSelected && <Check className="h-4 w-4 text-sky-500" />}
                  </div>
                )
              })}
            </div>
            {filters.industries.length > 0 && (
              <Button
                variant="ghost"
                size="sm"
                className="w-full mt-2 text-muted-foreground"
                onClick={() => onFiltersChange({ ...filters, industries: [] })}
              >
                Clear selection
              </Button>
            )}
          </PopoverContent>
        </Popover>
      )}

      {/* Source Filter */}
      <Popover>
        <PopoverTrigger asChild>
          <Button variant="outline" size="sm" className="h-9 gap-2">
            <Sparkles className="h-4 w-4" />
            {getMultiSelectLabel(filters.sources, "All Sources", "Source", "Sources")}
            <ChevronDown className="h-3 w-3 opacity-50" />
          </Button>
        </PopoverTrigger>
        <PopoverContent className="w-56 p-2" align="start">
          <div className="space-y-1 max-h-64 overflow-y-auto">
            {allSources.map((source) => {
              const isSelected = filters.sources.includes(source)
              return (
                <div
                  key={source}
                  className="flex items-center gap-2 p-2 rounded-md hover:bg-muted cursor-pointer"
                  onClick={() => toggleArrayFilter("sources", source)}
                >
                  <Checkbox checked={isSelected} />
                  <span className="text-sm truncate flex-1">{source}</span>
                  {isSelected && <Check className="h-4 w-4 text-sky-500" />}
                </div>
              )
            })}
          </div>
          {filters.sources.length > 0 && (
            <Button
              variant="ghost"
              size="sm"
              className="w-full mt-2 text-muted-foreground"
              onClick={() => onFiltersChange({ ...filters, sources: [] })}
            >
              Clear selection
            </Button>
          )}
        </PopoverContent>
      </Popover>

      {/* Location Filter (State/Country) */}
      {(availableStates.length > 0 || availableCountries.length > 0) && (
        <Popover>
          <PopoverTrigger asChild>
            <Button variant="outline" size="sm" className="h-9 gap-2">
              <MapPin className="h-4 w-4" />
              {filters.states.length > 0 || filters.countries.length > 0
                ? `${filters.states.length + filters.countries.length} Locations`
                : "All Locations"}
              <ChevronDown className="h-3 w-3 opacity-50" />
            </Button>
          </PopoverTrigger>
          <PopoverContent className="w-72 p-2" align="start">
            <div className="space-y-3">
              {availableCountries.length > 0 && (
                <div>
                  <Label className="text-xs text-muted-foreground uppercase tracking-wider">Countries</Label>
                  <div className="mt-1 space-y-1 max-h-32 overflow-y-auto">
                    {availableCountries.map((country) => {
                      const isSelected = filters.countries.includes(country)
                      return (
                        <div
                          key={country}
                          className="flex items-center gap-2 p-2 rounded-md hover:bg-muted cursor-pointer"
                          onClick={() => toggleArrayFilter("countries", country)}
                        >
                          <Checkbox checked={isSelected} />
                          <span className="text-sm truncate flex-1">{country}</span>
                          {isSelected && <Check className="h-4 w-4 text-sky-500" />}
                        </div>
                      )
                    })}
                  </div>
                </div>
              )}
              {availableStates.length > 0 && (
                <div>
                  <Label className="text-xs text-muted-foreground uppercase tracking-wider">States/Regions</Label>
                  <div className="mt-1 space-y-1 max-h-32 overflow-y-auto">
                    {availableStates.map((state) => {
                      const isSelected = filters.states.includes(state)
                      return (
                        <div
                          key={state}
                          className="flex items-center gap-2 p-2 rounded-md hover:bg-muted cursor-pointer"
                          onClick={() => toggleArrayFilter("states", state)}
                        >
                          <Checkbox checked={isSelected} />
                          <span className="text-sm truncate flex-1">{state}</span>
                          {isSelected && <Check className="h-4 w-4 text-sky-500" />}
                        </div>
                      )
                    })}
                  </div>
                </div>
              )}
            </div>
            {(filters.states.length > 0 || filters.countries.length > 0) && (
              <Button
                variant="ghost"
                size="sm"
                className="w-full mt-2 text-muted-foreground"
                onClick={() => onFiltersChange({ ...filters, states: [], countries: [] })}
              >
                Clear selection
              </Button>
            )}
          </PopoverContent>
        </Popover>
      )}

      {/* Assigned User Filter */}
      {members.length > 0 && (
        <Popover>
          <PopoverTrigger asChild>
            <Button variant="outline" size="sm" className="h-9 gap-2">
              <Users className="h-4 w-4" />
              {getMultiSelectLabel(
                filters.assignedTo.map(id => {
                  const member = members.find(m => m.profileId === id)
                  return member?.name || id
                }),
                "All Users",
                "User",
                "Users"
              )}
              <ChevronDown className="h-3 w-3 opacity-50" />
            </Button>
          </PopoverTrigger>
          <PopoverContent className="w-64 p-2" align="start">
            <div className="space-y-1">
              {members.length === 0 ? (
                <p className="text-sm text-muted-foreground p-2">No team members</p>
              ) : (
                members.map((member) => {
                  const isSelected = filters.assignedTo.includes(member.profileId)
                  return (
                    <div
                      key={member.profileId}
                      className="flex items-center gap-2 p-2 rounded-md hover:bg-muted cursor-pointer"
                      onClick={() => toggleArrayFilter("assignedTo", member.profileId)}
                    >
                      <Checkbox checked={isSelected} />
                      <Avatar className="h-6 w-6">
                        <AvatarImage src={member.avatarUrl || undefined} />
                        <AvatarFallback className="text-xs">
                          {member.name?.charAt(0)?.toUpperCase() || "?"}
                        </AvatarFallback>
                      </Avatar>
                      <span className="text-sm truncate flex-1">
                        {member.displayName || member.name}
                      </span>
                      {isSelected && <Check className="h-4 w-4 text-sky-500" />}
                    </div>
                  )
                })
              )}
            </div>
            {filters.assignedTo.length > 0 && (
              <Button
                variant="ghost"
                size="sm"
                className="w-full mt-2 text-muted-foreground"
                onClick={() => onFiltersChange({ ...filters, assignedTo: [] })}
              >
                Clear selection
              </Button>
            )}
          </PopoverContent>
        </Popover>
      )}

      {/* Tags Filter */}
      {tags.length > 0 && (
        <Popover>
          <PopoverTrigger asChild>
            <Button variant="outline" size="sm" className="h-9 gap-2">
              <Tag className="h-4 w-4" />
              {filters.tagIds.length === 0
                ? "All Tags"
                : filters.tagIds.length === 1
                  ? tags.find(t => t.id === filters.tagIds[0])?.name || "1 Tag"
                  : `${filters.tagIds.length} Tags`}
              <ChevronDown className="h-3 w-3 opacity-50" />
            </Button>
          </PopoverTrigger>
          <PopoverContent className="w-64 p-2" align="start">
            <div className="space-y-1 max-h-64 overflow-y-auto">
              {tags.map((tag) => {
                const isSelected = filters.tagIds.includes(tag.id)
                return (
                  <div
                    key={tag.id}
                    className="flex items-center gap-2 p-2 rounded-md hover:bg-muted cursor-pointer"
                    onClick={() => toggleArrayFilter("tagIds", tag.id)}
                  >
                    <Checkbox checked={isSelected} />
                    <Badge
                      variant="outline"
                      className="text-xs"
                      style={{
                        borderColor: tag.color,
                        backgroundColor: `${tag.color}20`,
                        color: tag.color,
                      }}
                    >
                      {tag.name}
                    </Badge>
                    <span className="flex-1" />
                    {isSelected && <Check className="h-4 w-4 text-sky-500" />}
                  </div>
                )
              })}
            </div>
            {filters.tagIds.length > 0 && (
              <Button
                variant="ghost"
                size="sm"
                className="w-full mt-2 text-muted-foreground"
                onClick={() => onFiltersChange({ ...filters, tagIds: [] })}
              >
                Clear selection
              </Button>
            )}
          </PopoverContent>
        </Popover>
      )}

      {/* Toggle Filters */}
      <div className="flex items-center gap-4 ml-2 border-l pl-4">
        <div className="flex items-center gap-2">
          <Switch
            id="has-opportunities"
            checked={filters.hasOpportunities === true}
            onCheckedChange={(checked) =>
              onFiltersChange({
                ...filters,
                hasOpportunities: checked ? true : null,
              })
            }
          />
          <Label htmlFor="has-opportunities" className="text-sm cursor-pointer">
            Has Opportunities
          </Label>
        </div>

        <div className="flex items-center gap-2">
          <Switch
            id="has-open-tasks"
            checked={filters.hasOpenTasks === true}
            onCheckedChange={(checked) =>
              onFiltersChange({
                ...filters,
                hasOpenTasks: checked ? true : null,
              })
            }
          />
          <Label htmlFor="has-open-tasks" className="text-sm cursor-pointer">
            Has Open Tasks
          </Label>
        </div>
      </div>

      {/* Spacer */}
      <div className="flex-1" />

      {/* Active filter count and clear button */}
      {activeFilterCount > 0 && (
        <Button
          variant="ghost"
          size="sm"
          className="h-9 gap-2 text-muted-foreground hover:text-foreground"
          onClick={clearAllFilters}
        >
          <Filter className="h-4 w-4" />
          {activeFilterCount} active
          <X className="h-3 w-3" />
        </Button>
      )}
    </div>
  )
}
