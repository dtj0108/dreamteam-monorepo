...
export function ProductSwitcher() {
  ...
  const currentProductId = useCurrentProduct()
  const { user } = useUser()
  const [showTutorial, setShowTutorial] = useState(false)

  useEffect(() => {
    const dismissed = localStorage.getItem("product-tutorial-dismissed")
    if (!dismissed) {
  ...
  const dismissTutorial = () => {
    localStorage.setItem("product-tutorial-dismissed", "true")
        ...
        <Popover open={showTutorial}>
          ...
          >
            ...
            {products.map((product) => {
              ...
              return (
                <DropdownMenuItem
                  key={product.id}
                  onClick={() => hasAccess && router.push(product.href)}
                ...
                >
                  <div className={`flex size-6 items-center justify-center rounded-md ${hasAccess ? "bg-muted" : "bg-muted"} text-base`}>
                    {hasAccess ? product.emoji : <Lock className="size-3 text-muted-foreground" />}
                  </div>
                  <div className="flex-1">
                    ...
                    <div className="text-xs text-muted-foreground">
                      {hasAccess ? product.description : "No access"}
                    </div>
                  </div>
          ...
          <PopoverContent side="right" align="start" className="w-64">
            <div className="space-y-3">
              <p className="font-medium">Switch Products</p>
              <p className="text-sm text-muted-foreground">
                You can switch between Finance, Sales, Team, and other products here.
              </p>
