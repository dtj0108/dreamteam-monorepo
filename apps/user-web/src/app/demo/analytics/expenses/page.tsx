...
export default function DemoExpensesPage() {
  const { overview, expensesByCategory, transactions } = useDemoData()
  ...
  return (
    <DemoDashboardLayout breadcrumbs={[{ label: "Analytics", href: "/demo/analytics" }, { label: "Expenses" }]}>
      <div className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-rose-600">{formatCurrency(overview.currentMonth.expenses)}</div>
            ...
            <div className="h-48 flex items-end justify-between gap-2 px-4">
              {overview.trend.map((month, i) => (
                <div key={i} className="flex-1 flex flex-col items-center gap-2">
                  <div 
                    className="w-full bg-rose-500 rounded-t min-h-[4px]"
                    style={{ height: `${(month.expenses / 80000) * 100}%` }}
