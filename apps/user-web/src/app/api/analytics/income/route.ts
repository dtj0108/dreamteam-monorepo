  ...
  try {
    ...
    if (accountIds.length === 0) {
      return NextResponse.json({
        ...
        summary: { totalIncome: 0, transactionCount: 0, sourceCount: 0, avgDaily: 0, avgMonthly: 0 },
        byCategory: [],
        topSources: [],
        monthlyTrend: [],
      })
    ...
    let query = supabase
      ...
      .gt('amount', 0) // Income is positive
    ...
    const avgDaily = totalIncome / dayCount
    const avgMonthly = totalIncome / Math.max(1, months.length)

    // Top sources
    const topSources = Object.values(byCategory)
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        sourceCount: Object.keys(byCategory).length,
        avgDaily,
        avgMonthly,
      },
