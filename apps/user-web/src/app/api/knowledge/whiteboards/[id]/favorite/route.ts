...
// POST /api/knowledge/whiteboards/[id]/favorite - Toggle favorite status
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (fetchError || !whiteboard) {
      console.error("Error fetching whiteboard:", fetchError)
      return NextResponse.json({ error: "Whiteboard not found" }, { status: 404 })
    }

    // Toggle favorite
    const currentFavorites: string[] = Array.isArray(whiteboard.is_favorited_by)
      ? whiteboard.is_favorited_by
      : []
    ...
    console.error("Error in POST /api/knowledge/whiteboards/[id]/favorite:", error)
