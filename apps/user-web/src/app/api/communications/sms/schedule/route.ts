...
export async function POST(request: NextRequest) {
  try {
    ...
    const { to, message, leadId, contactId, fromNumberId, scheduledFor } = body
    ...
    if (insertError) {
      console.error('Error creating scheduled SMS:', insertError)
      return NextResponse.json({
        error: `Failed to schedule SMS: ${insertError.message}`,
